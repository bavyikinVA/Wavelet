...
CPU и GPU сейчас используют разные типы чисел на части этапов:

- CPU для 1D в основном использует `float64`;
- GPU для 1D использует `float32`;
- некоторые последующие GPU-этапы также используют `float32`.

Если при расчёте CWT на GPU возникает ошибка, программа может продолжить расчёт на CPU.

Для строгой научной воспроизводимости в дальнейшем необходимо сохранять в манифесте:

- выбранное вычислительное устройство;
- фактически использованное устройство;
- типы чисел;
- сведения о переходе GPU → CPU.
